using Xamarin.Forms;
using Covid19.Models.CaseDetails;
...
using Covid19.Models.ReportData;
using System.Collections.Generic;

namespace Covid19.ViewModels.CaseDetails
{
    /// <summary>
    ...
    public class MyAddressViewModel : BaseViewModel
    {
        ...
        public List<CasesTimeSeries> DisplayData()
        {
            try
            {
                var Service = new Service12();
                var t = Task.Run(() => Service.GetResponse(new Uri("https://api.covid19india.org/data.json")));
                t.Wait();
                if (t.Result != null)
                {
                    var status = t.Result;
                }
                DataResource Data1 = JsonConvert.DeserializeObject<DataResource>(t.Result);

                List<CasesTimeSeries> casesTimes = new List<CasesTimeSeries>();
                casesTimes = Data1.cases_time_series;
